...
namespace TilePaletteLayoutStudio
{
    ...
    internal static class TilePaletteBuilder
    {
        ...
        private static void EnsurePaletteWindowClosed()
        {
            Type paletteWindowType = Type.GetType(
                "UnityEditor.Tilemaps.GridPaintPaletteWindow, Unity.2D.Tilemap.Editor");
            if (paletteWindowType == null) return;
            if (Resources.FindObjectsOfTypeAll(paletteWindowType).OfType<EditorWindow>().Any())
                throw new InvalidOperationException(
                    "Close the Unity Tile Palette window before Build, then try again.");
        }
